    ...
    <div className="layout-stack company-decisions">
      ...
      >
        <p className="finance-hint">
          Capture the choice, weigh the options, set a date. Close the loop.
        </p>
        ...
        {visible.length === 0 && (
          <p className="finance-empty">
            {filter === 'open'
              ? 'No open loops. Add a decision when something needs a call.'
              : filter === 'decided'
        ...
        <ul className="company-decisions-list">
          {visible.map((decision) => {
            ...
            const isExpanded = expandedId === decision.id || editingId === decision.id
              ...
              >
                {editingId === decision.id ? (
                  ...
                  <>
                    <button
                      type="button"
                      className="company-decision-head"
                      onClick={() =>
                        setExpandedId((id) => (id === decision.id ? null : decision.id))
                      }
                      aria-expanded={isExpanded}
                    >
                      <div className="company-decision-head-main">
                        ...
                        <div className="company-decision-badges">
                          ...
                          <span className="company-decision-opt-count">
                            {decision.options.length} option
                            {decision.options.length === 1 ? '' : 's'}
                          </span>
                        </div>
                      </div>
                      <span className={`company-decision-chevron${isExpanded ? ' open' : ''}`} aria-hidden>
                        ▾
                      </span>
                    </button>
